...
const SchedulePage = () => {
  ...
  // flatten events into array for MonthCalendar
  const eventsArray = Object.entries(events).flatMap(([dk, arr]) =>
    (arr || []).map((ev) => ({ ...ev, date: dk }))
  );
      ...
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-transparent p-2 flex justify-center">
          <div className="w-full">
            <MonthCalendar onDateClick={handleDateClick} events={eventsArray} small={false} />
          </div>
        </div>
      </div>
