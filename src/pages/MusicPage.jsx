...
const MusicPage = () => {
  ...
  useEffect(() => {
    ...
    if (storedLogs) setMusicLogs(JSON.parse(storedLogs));
  }, []);

  const resetForm = () => {
    setDate(""); setTitle(""); setMemo(""); setUrl(""); setThumbnail(null); setEditIndex(null);
  };

  const handleSave = () => {
    if (!date || !title) {
      setMessage("日付とタイトルは必須です。"); return;
    }
    ...
    try { refreshFromStorages(); } catch (e) {}
  ...
  const handleEdit = (index) => {
    ...
    setDate(entry.date); setTitle(entry.title); setMemo(entry.memo); setUrl(entry.url);
    setThumbnail(entry.thumbnail); setEditIndex(index);
  };
                ...
                {musicLogs[selectedCategory].map((entry, index) => (
                  <div key={index} className="bg-white shadow-md rounded-xl overflow-hidden">
                    <div className="p-4 flex flex-col h-full">
                      ...
                      {entry.url && (
                        <a href={entry.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline mt-2">▶ 楽曲リンク</a>
                      )}
